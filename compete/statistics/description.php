<?php
require_once "../content/require.php";

if (isset($_GET['solution'], $_GET['id'])) {
    $state = $db->prepare("select * from problem_data where id = ?");
    $state->execute(array($_GET['id']));
    $problem = $state->fetchAll(PDO::FETCH_ASSOC);
    if (count($problem) > 0) {
        $problem = $problem[0];
        ?>
        <a class="btn btn-primary btn-lg" href="print.php?id=<? echo $problem['id']; ?>&solution=yes" target="_blank">Print Solution</a>
        <pre><code class="java github"><? echo htmlspecialchars($problem['problem_code']); ?></code></pre>
        <?
    } else {
        ?>
        <pre>Problem ID not found.</pre>
        <?
    }
} else if (isset($_GET['id'])) {
    $state = $db->prepare("select * from problem_data where id = ?");
    $state->execute(array($_GET['id']));
    $problem = $state->fetchAll(PDO::FETCH_ASSOC);
    if (count($problem) > 0) {
        $problem = $problem[0];
        ?>
        <h1 class="text-center"><? echo $problem['problem_title']; ?></h1>
        <a class="btn btn-primary pull-right btn-lg" href="print.php?id=<? echo $problem['id']; ?>" target="_blank">Print Problem</a>
        <? echo $problem['problem_description']; ?>
        <hr/>
        <h2>SAMPLE INPUT: </h2>
        <pre class="well well-sm"><? echo $problem['problem_sample_input']; ?></pre>

        <h2>SAMPLE OUTPUT: </h2>
        <pre class="well well-sm"><? echo $problem['problem_sample_output']; ?></pre>
        <?
    } else {
        ?>
        <pre>Problem ID not found.</pre>
        <?
    }
} else {
    ?>
    <pre>Problem ID not found.</pre>
    <?
}